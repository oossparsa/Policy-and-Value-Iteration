...
def iteratePolicy():
    ...
    while GoalIsReached:
        ...
        for i in range(81):
            ...
            expectedValue = 0.625*(rewardR + policyEvaluationGrid[ysR][xsR])\
                            +0.125*(rewardL + policyEvaluationGrid[ysL][xsL])\
                            +0.125*(rewardD + policyEvaluationGrid[ysD][xsD])\
                            +0.125*(rewardU + policyEvaluationGrid[ysU][xsU])
...
def printGreedy():
    ...
    for i in range(81):
        ...
        if(xs==8 and ys==0):
            if(policyEvaluationGrid[0][7] > policyEvaluationGrid[1][8]):
                gridPath[ys][xs] = '←'
            elif(policyEvaluationGrid[0][7] < policyEvaluationGrid[1][8]):
                gridPath[ys][xs] = '↓'
            else:
                gridPath[ys][xs] = '←,↓'
            continue
        if(xs==0 and ys==0):
            if(policyEvaluationGrid[0][1] > policyEvaluationGrid[1][0]):
                gridPath[ys][xs] = '→'
            elif(policyEvaluationGrid[0][1] < policyEvaluationGrid[1][0]):
                gridPath[ys][xs] = '↓'
            else:
                gridPath[ys][xs] = '→,↓'
            continue
        if(xs==0 and ys==8):
            if(policyEvaluationGrid[8][1] > policyEvaluationGrid[7][0]):
                gridPath[ys][xs] = '→'
            elif(policyEvaluationGrid[8][1] < policyEvaluationGrid[7][0]):
                gridPath[ys][xs] = '↑'
            else:
                gridPath[ys][xs] = '→,↑'
            continue
        if(xs==8 and ys==8):
            if(policyEvaluationGrid[8][7] > policyEvaluationGrid[7][8]):
                gridPath[ys][xs] = '←'
            elif(policyEvaluationGrid[8][7] < policyEvaluationGrid[7][8]):
                gridPath[ys][xs] = '↑'
            else:
                gridPath[ys][xs] = '←,↑'
            continue
        ...
        if(xs==0):
            maxNeighbour = max(policyEvaluationGrid[ys][xs + 1],
